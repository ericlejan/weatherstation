...
# Main loop
	if option1.strip() == 'start' :
		...
		while True :
			...
			if nbmes == 0 :
				print 'UV Index :\tHumid(BME) : \tHumid(HTU) : \tHumid(HTUcor) \tTemp(MCP) : \tTemp(BMP) : \tTemp(BME) : \tTemp(MPL) : \tTemp(HTU) : \tPress(BMP) : \tPress(BME) : \tPress(MPL) : \tPress(SealevelMPL) : ' 
				nbmes += 1
			elif nbmes == 9 :
				nbmes = 0
			else :
				nbmes += 1
			print '%.2f\t\t' %uvi + '%2.1f\t\t' %hygroBME + '%2.1f\t\t' %hygroHTU + '%2.1f\t\t' %rhHTU + '%.4f °C\t' %tempHR + '%.2f °C\t' %tempBMP +  '%.2f °C\t' %tempBME + '%.2f °C\t' %tempMPL + '%.2f °C\t' %tempHTU + '%.2f hPa\t' %pressBMP + '%.2f hPa\t' %pressBME + '%.2f hPa\t' %pressMPL  + '%.2f hPa\t' %seaLevelPress
